...
public class BST<T extends Comparable<T>> {
	...
	private Node remove(T data, Node node) {
		...
		if (data.compareTo(node.data) < 0) { // smaller than node.data
			...
			if (node.left == null && node.right == null) { // if it's a leaf, remove directly
				...
				// replace the node with biggest value from the left subtree
	...
	private void heightTraverseString(Queue<Node> q, StringBuilder heightTraverse) {
		while (!q.isEmpty()) {
			...
			//put the 2 node of next height into queue
			if (node.left != null) {
				q.enqueue(node.left);
			}
			if (node.right != null) {
	...
	private T sharedPrecursor(T data1, T data2, Node currLevel) {
		...
		if(currLevel == null) {	//base case
			return null;
		}
		
		//if matched one of the 2 data
		if(currLevel.data.compareTo(data1) == 0 || currLevel.data.compareTo(data2) == 0) {
		...
		//if both left and right found the data
		if (left != null && right != null) {
			//this node is their precursor
            return currLevel.data;
        }
		
		//if only found one data or nothing
